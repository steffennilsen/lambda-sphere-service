...
/**
 * https://github.com/nouvadam/spherical-blue-noise
 */
use glam::*;
use rand::Rng;
...
const NORTH_POLE: Vec3 = Vec3 {
    x: (0 as f32),
    y: (1 as f32),
    z: (0 as f32),
};

const SOUTH_POLE: Vec3 = Vec3 {
    x: (0 as f32),
    y: (-1 as f32),
    z: (0 as f32),
};
...
impl BlueNoiseSphere {
    ...
    pub fn new_with_poles<R: Rng + ?Sized>(num_of_points: u32, rng: &mut R) -> Self {
        Self::new_with_poles_raw(num_of_points, rng).advance_multiple(
            16,
            0.999_383_57_f32.powi(num_of_points as i32) / 4.0 + 0.01,
            0.8,
        )
    ...
    pub fn new_with_poles_raw<R: Rng + ?Sized>(num_of_points: u32, rng: &mut R) -> Self {
        let mut particles: Vec<Vec3> = (0..num_of_points - 2)
            .map(|_n| {
                let sample = UnitSphere.sample(rng);
                Vec3::new(sample[0], sample[1], sample[2])
            })
        ...
        particles.insert(0, NORTH_POLE);
        particles.insert((num_of_points - 1) as usize, SOUTH_POLE);

        BlueNoiseSphere { particles }
    }
        ...
        BlueNoiseSphere {
            particles: self
                ...
                .map(|current_particle| {
                    if current_particle == &NORTH_POLE {
                        return NORTH_POLE;
                    }

                    if current_particle == &SOUTH_POLE {
                        return SOUTH_POLE;
                    }
